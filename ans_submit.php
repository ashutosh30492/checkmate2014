<?php
//TODO
//a notification bar to show the result of last attempt
//round of the percentage
//showing the question no.
//end of TODO
...
<?php
	...
	if(empty($error)){
		...
		if($question['answer'] == $_GET['ans']){
			...
			//incrementing points need to change
			...
			// $query_result = mysql_query("update `user_point` set `question_array`='{$_SESSION['question_array']}' where `user_id` = {$_SESSION['user_id']}");
			// confirm_query($result_query);

			$result_query = mysql_query("update `q_count` set `no_sub` = `no_sub`+1 where `q_no` = {$qno}");
			confirm_query($result_query);
			
			//decrementing the points need to change
			...
			$query_result = mysql_query("update `user_point` set `point` = '{$_SESSION['point']}',
							`question_array`='{$_SESSION['question_array']}' where `user_id` = {$_SESSION['user_id']}");
			confirm_query($result_query);
